...
#include "RemoteDebug.h"  //DGF added

extern RemoteDebug Debug; //DGF added

unsigned long SequenceNo = 0;
bool isHandlingInvalidSequenceNo = false;
byte ProcedureSequenceNo = 5; // Must be 5-9 for MEP
boolean AlertSeqenceActive = false;

// DGF 06-DEC-2023 Meterconfig changed to providerMeterConfig (becaue of name conflict with MeterConfig in AmsToMqttBridge file)
MeterConfigStruct providerMeterConfig;
// DGF
...
/***********************************************************************************************************************************************************
 * Purpose: Queue request for seding to meter
 * History: D.Geisenhoff 04-DEC-2023 This comment added
 * *********************************************************************************************************************************************************/
void queueRequest(String request,String key,MEPQueueStruct *MEPQueue,byte *MEPQueueNextIndex,NextActionEnum NextAction)
{
  ...
  //Serial.printf("Queued request at index %i %s\r\n",*MEPQueueNextIndex,request.substring(6,10));
  Serial.printf("queueRequest: Queued request at index %i %s SqNo: %i\r\n", *MEPQueueNextIndex, request.c_str(), SequenceNo - 1);
  // Serial.print("Request: ");
  // Serial.println(bytesToHexString(MEPQueue[*MEPQueueNextIndex].Request,MEPQueue[*MEPQueueNextIndex].RequestLength,true));
  IncreaseMEPQueueIndex(MEPQueueNextIndex);
}


/***********************************************************************************************************************************************************
 * Purpose: Increase sequence number of message to meter
 * History: D.Geisenhoff 04-DEC-2023 This comment added
 * *********************************************************************************************************************************************************/
void IncreaseProcedureSequenceNo(byte *procSequenceNo)
{
  (*procSequenceNo)++;
  if(*procSequenceNo>9)
  {
...
{
  if(ReplyLength < 15) // DGF < instead of <=
  { 
    if(Debug.isActive(RemoteDebug::ERROR)) Debug.printf_P(PSTR("Returned, because response length (%i) too small, expected: 15.\n"), ReplyLength);  //DGF added
    return false;
  }
...
{
  if(ReplyLength < 9) // DGF < instead of <=
  {
    if(Debug.isActive(RemoteDebug::ERROR)) Debug.printf_P(PSTR("%sReturned, because response length (%i) too small, expected: 9.%s\n"), COLOR_RED, ReplyLength,COLOR_RESET);  //DGF added
    return false;
  }
  *BT21_0 = Reply[0];
  *BT21_1 = Reply[1];
...
/***********************************************************************************************************************************************************
 * Purpose: Decode register BT23
 * History: D.Geisenhoff 06-DEC-2023 This comment added
 * *********************************************************************************************************************************************************/
...
{
  //Serial.printf("Reply length: %i\n",ReplyLength);
  byte A = ((providerMeterConfig.BT21_0 & 0b00000100) == 0b00000100) ? 1 : 0; // BT21_0_2
  // DGF 14-DEC-2023
  if (ReplyLength == 12 + 4)
    //tier (does not have first byte A, only total has A)
    A = 0;
  //DGF
  if (ReplyLength < A + 12 + 4) // DGF < instead of <=
  {
    Serial.println("Returnig");
    if (Debug.isActive(RemoteDebug::ERROR))
      Debug.printf_P(PSTR("%sReturned, because response length (%i) too small, expected: %i.%s\n"), COLOR_RED, ReplyLength, A + 16, COLOR_RESET); // DGF added
    return false;
  }
...
{
  byte A = ((providerMeterConfig.BT21_0 & 0b01000000) == 0b01000000) ? 3 : 0; // BT21_0_6
  byte Offset = (A + 4) * providerMeterConfig.BT21_8;
  //Serial.print('Offset: ');
  //Serial.println(Offset.toString());
  if (ReplyLength < Offset + 25 * 4) // DGF < instead of <=
  { // EHorvat: We just check for 25 datapoints from BT28 (original was 54*4)
    if(Debug.isActive(RemoteDebug::ERROR)) Debug.printf_P(PSTR("%sReturned, because response length (%i) too small, expected: %i.%s\n"), COLOR_RED, ReplyLength,Offset,COLOR_RESET);  //DGF added
    return false;
  }
  // All these are type NI_FMAT1 = 32 bit signed integer
...
{
  if(ReplyLength < 7) // DGF < instead of <=
  {
    if(Debug.isActive(RemoteDebug::ERROR)) Debug.printf_P(PSTR("%sReturned, because response length (%i) too small, expected: 7.%s\n"), COLOR_RED, ReplyLength,COLOR_RESET);  //DGF added
    return false;
  }
  *year_     = Reply[0];
  *month_    = Reply[1];
...
/***********************************************************************************************************************************************************
 * Purpose: Read register 0x0037 (current tariff).
 * History: D.Geisenhoff  04-DEC-2023 Created
 * *********************************************************************************************************************************************************/
boolean Decode0x0037(unsigned long ReplyLength,byte Reply[],byte *tariffId)
{
  if(ReplyLength < 9) // DGF < instead of <=
  {
    if(Debug.isActive(RemoteDebug::ERROR)) Debug.printf_P(PSTR("%sReturned, because response length (%i) too small, expected: 9.%s\n"), COLOR_RED, ReplyLength,COLOR_RESET);  //DGF added
...
{
  if(ReplyLength < 138) // DGF < instead of <=
  {
    if(Debug.isActive(RemoteDebug::ERROR)) Debug.printf_P(PSTR("%sReturned, because response length (%i) too small, expected: 138.%s\n"), COLOR_RED, ReplyLength,COLOR_RESET);  //DGF added
    return false;
  }
...
boolean Decode0x080B(unsigned long ReplyLength,byte Reply[],byte *ET11_0,byte *ET11_1,byte *ET11_2,byte *ET11_3,byte *ET11_4,word *ET11_5,word *ET11_7,word *ET11_9,byte *ET11_11,
                     ...
                     word *ET11_30)
{
  if(ReplyLength < 31) // DGF < instead of <=
  {
    if(Debug.isActive(RemoteDebug::ERROR)) Debug.printf_P(PSTR("%sReturned, because response length (%i) too small, expected: 31.%s\n"), COLOR_RED, ReplyLength,COLOR_RESET);  //DGF added
    return false;
  }
  *ET11_0 = Reply[0];
  *ET11_1 = Reply[1];
                     ...
                     byte *ET13_MEP_7,byte *ET13_MEP_8,byte ET13_MEP_9[])
{
  ...
  if(ReplyLength < providerMeterConfig.ET11_0 * providerMeterConfig.ET11_1) // DGF < instead of <=
  {
    if(Debug.isActive(RemoteDebug::ERROR)) Debug.printf_P(PSTR("%sReturned, because response length (%i) too small, expected: %i.%s\n"), COLOR_RED, ReplyLength,providerMeterConfig.ET11_0 * providerMeterConfig.ET11_1,COLOR_RESET);  //DGF added
    return false;
  }

  for(byte i = 0;i<4;i++)
  {
    Offset = i * providerMeterConfig.ET11_1;
  ...
  Offset = 4 * providerMeterConfig.ET11_1;
  ...
  *ET13_MEP_8 = Reply[Offset + 8];
  memcpy(ET13_MEP_9, Reply + Offset + 9, providerMeterConfig.ET11_1 - 9);
  return true;
}
...
boolean Decode0x0832(unsigned long ReplyLength,byte Reply[],byte ET50_0[],byte *ET50_30,word ET50_31[],long ET50_43[],byte *ET50_67)
{
  if(ReplyLength < 31) // DGF < instead of <=
  {
    if(Debug.isActive(RemoteDebug::ERROR)) Debug.printf_P(PSTR("Returned, because response length (%i) too small, expected: 31.\n"), ReplyLength);  //DGF added
    return false;
  }
...
{
  if(ReplyLength < 3) // DGF < instead of <=
  {
    if(Debug.isActive(RemoteDebug::ERROR)) Debug.printf_P(PSTR("%sReturned, because response length (%i) too small, expected: 3.%s\n"), COLOR_RED, COLOR_RESET);  //DGF added
    return false;
  }
  switch(Reply[3]) 
  {
...
/***********************************************************************************************************************************************************
 * Purpose: Read registers and store into data structure.
 * History: D.Geisenhoff  04-DEC-2023 This comment added
 *          D.Geisenhoff  14-DEC-2023 Tiers added   
 * *********************************************************************************************************************************************************/
String ReplyData2String(MEPQueueStruct *MEPQueue,byte MEPQueueReplyIndex, boolean UpdateDataStructures)
{
  // Serial.print("ReplyDataToString: Response Register:");
  // Serial.println(MEPQueue[MEPQueueReplyIndex].Request[3] * 256 + MEPQueue[MEPQueueReplyIndex].Request[4]);
  if(MEPQueue[MEPQueueReplyIndex].RequestLength < 8)
    return "";

  // Serial.print("Reply:");
  // String reply = bytesToHexString(MEPQueue[MEPQueueReplyIndex].Reply+5, MEPQueue[MEPQueueReplyIndex].ReplyLength-5, true);
  // Serial.println(reply);
  if (MEPQueue[MEPQueueReplyIndex].Reply[2] != 0x00)
    return "";
  isHandlingInvalidSequenceNo = false;
  // DGF 06-DEC-2023 Offset does not necessairily have to be 0
  if (MEPQueue[MEPQueueReplyIndex].Request[2] == 0x30 || MEPQueue[MEPQueueReplyIndex].Request[2] == 0x3f) 
  ...
  //DGF
  {
    unsigned long responseDataLength = MEPQueue[MEPQueueReplyIndex].ReplyLength - 13; // DGF 2 record length, 1 error code, 2 data length, 8 digest)
    switch (MEPQueue[MEPQueueReplyIndex].Request[3] * 256 + MEPQueue[MEPQueueReplyIndex].Request[4])
    {
      case 0x0001:
      char Manufacturer[5]; // BT01: General Manufacturer Information
      char Model[9];
      ...
      if (Decode0x0001(responseDataLength, MEPQueue[MEPQueueReplyIndex].Reply + 5, // DGF ReplyLength replaced by responseDataLength
    ...
    case 0x0015:
      if (Decode0x0015(responseDataLength, MEPQueue[MEPQueueReplyIndex].Reply + 5, &providerMeterConfig.BT21_0, &providerMeterConfig.BT21_1, &providerMeterConfig.BT21_2, &providerMeterConfig.BT21_3, // DGF ReplyLength replaced by responseDataLength
                       &providerMeterConfig.BT21_4, &providerMeterConfig.BT21_5, &providerMeterConfig.BT21_6, &providerMeterConfig.BT21_7, &providerMeterConfig.BT21_8, &providerMeterConfig.BT21_9))
      {
        return "Bool. flags 0: " + Byte2Binary(providerMeterConfig.BT21_0) + "<br>" +
               "Bool. flags 1: " + Byte2Binary(providerMeterConfig.BT21_1) + "<br>" +
               "No. of Self-reads: " + String(providerMeterConfig.BT21_2) + "<br>" +
               "No. of Summations: " + String(providerMeterConfig.BT21_3) + "<br>" +
               "No. of Demands: " + String(providerMeterConfig.BT21_4) + "<br>" +
               "No. of Coincident Values: " + String(providerMeterConfig.BT21_5) + "<br>" +
               "No. of Occurrences: " + String(providerMeterConfig.BT21_6) + "<br>" +
               "No. of Tiers: " + String(providerMeterConfig.BT21_7) + "<br>" +
               "No. of Present Demands: " + String(providerMeterConfig.BT21_8) + "<br>" +
               "No. of Present Values: " + String(providerMeterConfig.BT21_9);
      }
      break;

    case 0x0017:
    {
      long FwdActiveWhL1L2L3;
      long RevActiveWhL1L2L3;
      long FwdReactiveWhL1L2L3;
      long RevReactiveWhL1L2L3;
      // DGF 06-DEC-2023 Read tier specific data of BT23 added
      const byte tierId = MEPQueue[MEPQueueReplyIndex].RegisterSpecificParam;
      //DGF
      if (Decode0x0017(responseDataLength, MEPQueue[MEPQueueReplyIndex].Reply + 5, &FwdActiveWhL1L2L3, &RevActiveWhL1L2L3, &FwdReactiveWhL1L2L3, &RevReactiveWhL1L2L3)) //DGF ReplyLength replaced by responseDataLength
      {
        ...
        if (UpdateDataStructures)
        {
          ...
          {
            // DGF Total (no tier)
            ConsumptionData.bt23_total.Fwd_Act_Wh = FwdActiveWhL1L2L3;
            ConsumptionData.bt23_total.Rev_Act_Wh = RevActiveWhL1L2L3;
            ConsumptionData.bt23_total.Fwd_React_Wh = FwdReactiveWhL1L2L3;
            ConsumptionData.bt23_total.Rev_React_Wh = RevReactiveWhL1L2L3;
          }
          else
          {
            //DGF tier 
            ConsumptionData.bt23_tiers[tierId].Fwd_Act_Wh = FwdActiveWhL1L2L3;
            ConsumptionData.bt23_tiers[tierId].Rev_Act_Wh = RevActiveWhL1L2L3;
            ConsumptionData.bt23_tiers[tierId].Fwd_React_Wh = FwdReactiveWhL1L2L3;
            ConsumptionData.bt23_tiers[tierId].Rev_React_Wh = RevReactiveWhL1L2L3;
          }
          //DGF
//          Serial.printf("Storing Fwd_Act_Wh=%lu and Rev_Act_Wh=%lu\r\n", ConsumptionData.BT23_Fwd_Act_Wh, ConsumptionData.BT23_Rev_Act_Wh);
    ...
    case 0x001C:
    {
      ...
      if (Decode0x001C(responseDataLength, MEPQueue[MEPQueueReplyIndex].Reply + 5, &FwdActiveWL1L2L3, &RevActiveWL1L2L3, &ImportReactiveVArL1L2L3, &ExportReactiveVArL1L2L3, //DGF ReplyLength replaced by responseDataLength
    ...
    case 0x0034:
      ...
      if (Decode0x0034(responseDataLength, MEPQueue[MEPQueueReplyIndex].Reply + 5, &year_, &month_, &day_, &hour_, &minute_, &second_, &DayOfWeek, &DST, &GMT, &TZ, &DSTApp)) //DGF ReplyLength replaced by responseDataLength
      {
        if (UpdateDataStructures)
        {
          setenv("TZ", "UTC", 1);
    ...
    // DGF 04-DEC-2023 Current tariff added
    case 0x0037:
      byte tariffId;

      if (Decode0x0037(responseDataLength, MEPQueue[MEPQueueReplyIndex].Reply + 5, &tariffId)) //DGF ReplyLength replaced by responseDataLength
      {
        if (UpdateDataStructures)
        {
          ConsumptionData.BT55_Current_Tier_Id = tariffId;
        }
        //Serial.print("Tariff id: ");
        //Serial.println(tariffId);
        return "Tariff id: " + String(tariffId);
      }
      break;
      // DGF
    ...
    case 0x0803:
    {
      char UtilitySerialNumber[31]; // ET03: Utility Information
      word ImageCRC;
      if (Decode0x0803(responseDataLength, MEPQueue[MEPQueueReplyIndex].Reply + 5, UtilitySerialNumber, &ImageCRC)) //DGF ReplyLength replaced by responseDataLength
      {
    ...
    case 0x080B:
      if (Decode0x080B(responseDataLength, MEPQueue[MEPQueueReplyIndex].Reply + 5, &providerMeterConfig.ET11_0, &providerMeterConfig.ET11_1, &providerMeterConfig.ET11_2, &providerMeterConfig.ET11_3, &providerMeterConfig.ET11_4, //DGF ReplyLength replaced by responseDataLength
                       &providerMeterConfig.ET11_5, &providerMeterConfig.ET11_7, &providerMeterConfig.ET11_9, &providerMeterConfig.ET11_11, &providerMeterConfig.ET11_12, &providerMeterConfig.ET11_13, &providerMeterConfig.ET11_18, &providerMeterConfig.ET11_19, &providerMeterConfig.ET11_20,
                       &providerMeterConfig.ET11_21, &providerMeterConfig.ET11_22, &providerMeterConfig.ET11_23, &providerMeterConfig.ET11_24, &providerMeterConfig.ET11_25, &providerMeterConfig.ET11_26, &providerMeterConfig.ET11_27, &providerMeterConfig.ET11_28, &providerMeterConfig.ET11_30))
      {
        return "No. of Devices: " + String(providerMeterConfig.ET11_0) + "<br>" +
               "Config Entry Size: " + String(providerMeterConfig.ET11_1) + "<br>" +
               "Status Entry Size: " + String(providerMeterConfig.ET11_2) + "<br>" +
               "On-demand Request Queue Size: " + String(providerMeterConfig.ET11_3) + "<br>" +
               "On-demand Request Entry Size: " + String(providerMeterConfig.ET11_4) + "<br>" +
               "Data Entry Size: " + String(providerMeterConfig.ET11_5) + "<br>" +
               "Transaction Request Length: " + String(providerMeterConfig.ET11_7) + "<br>" +
               "Transaction Response Length: " + String(providerMeterConfig.ET11_9) + "<br>" +
               "On-demand Write Entry Size: " + String(providerMeterConfig.ET11_11) + "<br>" +
               "Phase Measurement Data Size: " + String(providerMeterConfig.ET11_12) + "<br>" +
               "Config Entry 2 Size: " + String(providerMeterConfig.ET11_13) + "<br>" +
               "No. of Group IDs: " + String(providerMeterConfig.ET11_18) + "<br>" +
               "No. of Harmonics: " + String(providerMeterConfig.ET11_19) + "<br>" +
               "M-Bus Multicast Message Length: " + String(providerMeterConfig.ET11_20) + "<br>" +
               "ET22 Alarm Size: " + String(providerMeterConfig.ET11_21) + "<br>" +
               "MEP Data Sources: " + String(providerMeterConfig.ET11_22) + "<br>" +
               "ET48 Entry Count: " + String(providerMeterConfig.ET11_23) + "<br>" +
               "Max Critical Event Bitmaps: " + String(providerMeterConfig.ET11_24) + "<br>" +
               "ET57 Entry Count: " + String(providerMeterConfig.ET11_25) + "<br>" +
               "Time-based Relay Switches: " + String(providerMeterConfig.ET11_26) + "<br>" +
               "MEP Data Alert Sources: " + String(providerMeterConfig.ET11_27) + "<br>" +
               "RAM-only Transaction Request Length: " + String(providerMeterConfig.ET11_28) + "<br>" +
               "RAM-only Transaction Response Length: " + String(providerMeterConfig.ET11_30);
      }
      break;
    case 0x080D:
    {
      String Message;
      if (Decode0x080D(responseDataLength, MEPQueue[MEPQueueReplyIndex].Reply + 5, providerMeterConfig.MBusInfo, &providerMeterConfig.ET13_MEP_0, &providerMeterConfig.ET13_MEP_1, &providerMeterConfig.ET13_MEP_2, //DGF ReplyLength replaced by responseDataLength
                       &providerMeterConfig.ET13_MEP_3, &providerMeterConfig.ET13_MEP_4, &providerMeterConfig.ET13_MEP_6, &providerMeterConfig.ET13_MEP_7, &providerMeterConfig.ET13_MEP_8, providerMeterConfig.ET13_MEP_9))
      {
        for (byte i = 0; i < 4; i++)
        {
          Message = Message + "M-Bus" + String(i + 1) + "-Scheduled Billing Read - Day: " + ScheduledDay2String(providerMeterConfig.MBusInfo[i].ET13_MBus_0) + "<br>" +
                    "M-Bus" + String(i + 1) + "-Scheduled Billing Read - Hour: " + String(providerMeterConfig.MBusInfo[i].ET13_MBus_1) + "<br>" +
                    "M-Bus" + String(i + 1) + "-Scheduled Billing Read - Minute: " + String(providerMeterConfig.MBusInfo[i].ET13_MBus_2) + "<br>" +
                    "M-Bus" + String(i + 1) + "-Scheduled Billing Read - Frequency: " + BillingReadFreq2String(providerMeterConfig.MBusInfo[i].ET13_MBus_3) + "<br>" +
                    "M-Bus" + String(i + 1) + "-Scheduled Status Read (M-Bus) - Frequency: " + String(providerMeterConfig.MBusInfo[i].ET13_MBus_4) + "<br>" +
                    "M-Bus" + String(i + 1) + "-Baud Rate: " + BaudRate2String(providerMeterConfig.MBusInfo[i].ET13_MBus_6) + "<br>" +
                    "M-Bus" + String(i + 1) + "-Device Alarm Bitmask: " + Byte2Binary(providerMeterConfig.MBusInfo[i].ET13_MBus_7) + "<br>" +
                    "M-Bus" + String(i + 1) + "-Customer ID: " + String(providerMeterConfig.MBusInfo[i].ET13_MBus_8) + "<br>" +
                    "M-Bus" + String(i + 1) + "-App Reset Parameter: " + String(providerMeterConfig.MBusInfo[i].ET13_MBus_28) + "<br>" +
                    "M-Bus" + String(i + 1) + "-Security Key: " + bytesToHexString(providerMeterConfig.MBusInfo[i].ET13_MBus_30, providerMeterConfig.MBusInfo[i].ET13_MBus_29, true) + "<br>" +
                    "M-Bus" + String(i + 1) + "-Ta: " + String(providerMeterConfig.MBusInfo[i].ET13_MBus_62) + "<br>" +
                    "M-Bus" + String(i + 1) + "-To: " + String(providerMeterConfig.MBusInfo[i].ET13_MBus_64) + "<br>";
        }
        return Message +
               "MEP-Scheduled Billing Read - Day: " + ScheduledDay2String(providerMeterConfig.ET13_MEP_0) + "<br>" +
               "MEP-Scheduled Billing Read - Hour: " + String(providerMeterConfig.ET13_MEP_1) + "<br>" +
               "MEP-Scheduled Billing Read - Minute: " + String(providerMeterConfig.ET13_MEP_2) + "<br>" +
               "MEP-Scheduled Billing Read - Frequency: " + BillingReadFreq2String(providerMeterConfig.ET13_MEP_3) + "<br>" +
               "MEP-Scheduled Status Read (M-Bus) - Frequency: " + String(providerMeterConfig.ET13_MEP_4) + "<br>" +
               "MEP-Baud Rate: " + BaudRate2String(providerMeterConfig.ET13_MEP_6) + "<br>" +
               "MEP-Device Alarm Bitmask: " + Byte2Binary(providerMeterConfig.ET13_MEP_7) + "<br>" +
               "MEP-Alert flags: " + Byte2Binary(providerMeterConfig.ET13_MEP_8) + "<br>" +
               "MEP-Non-urgent Downlink Data: " + bytesToHexString(providerMeterConfig.ET13_MEP_9, 57, true);
      }
      break;
      }
      ...
      case 0x0832: 
      {
        if(Decode0x0832(responseDataLength,MEPQueue[MEPQueueReplyIndex].Reply+5,providerMeterConfig.ET50_0,&providerMeterConfig.ET50_30,providerMeterConfig.ET50_31,providerMeterConfig.ET50_43,&providerMeterConfig.ET50_67))  //DGF ReplyLength replaced by responseDataLength
        {
                     return "Identification String (MEP ID): " + bytesToHexString(providerMeterConfig.ET50_0,30,true) + "<br>" +
                            "MEP Flags: " + Byte2Binary(providerMeterConfig.ET50_30) + "<br>" +
                            "Data Sources Icon Display Control: " + Word2Binary(providerMeterConfig.ET50_31[0]) + ", " + Word2Binary(providerMeterConfig.ET50_31[1]) + ", " + Word2Binary(providerMeterConfig.ET50_31[2]) + ", " + 
                                                                    Word2Binary(providerMeterConfig.ET50_31[3]) + ", " + Word2Binary(providerMeterConfig.ET50_31[4]) + ", " + Word2Binary(providerMeterConfig.ET50_31[5]) + "<br>" +
                            "Data for Display/Load Profile Sources: " + String(providerMeterConfig.ET50_43[0]) + ", " + String(providerMeterConfig.ET50_43[1]) + ", " + String(providerMeterConfig.ET50_43[2]) + ", " + 
                                                                        String(providerMeterConfig.ET50_43[3]) + ", " + String(providerMeterConfig.ET50_43[4]) + ", " + String(providerMeterConfig.ET50_43[5]) + "<br>" +
                            "MEP Icon Display Control: " + Byte2Binary(providerMeterConfig.ET50_67);
        }
        break;
      }
      ...
      {
        String Message;
        if(Decode0x0855(responseDataLength,MEPQueue[MEPQueueReplyIndex].Reply+5,&Message)) //DGF ReplyLength replaced by responseDataLength
        {
          return Message;
        }
        break;
...
String ReplyPackageToMessage(MEPQueueStruct *MEPQueue,byte MEPQueueReplyIndex)
{
  ...
  switch(MEPQueue[MEPQueueReplyIndex].Request[3]*256 + MEPQueue[MEPQueueReplyIndex].Request[4]) {
    ...
    case 0x0034: Message = Message + "0x0034 BT52 UTC Clock";
                 break;
    // DGF 04-dec-2023 Register 55 added (for tariff id)
    case 0x0037: Message = Message + "0x0037 BT55 Local Clock State";
                 break;
    // DGF
...
void HandleInvalidSequenceNumber(String key,MEPQueueStruct *MEPQueue,byte MEPQueueReplyIndex, byte *MEPQueueNextIndex)
{
  String Request;
  //DGF
  //Serial.printf("HandleInvalidSequenceNumber\r\n");
  ...
  {  
    if (isHandlingInvalidSequenceNo)
    return;
    isHandlingInvalidSequenceNo = true;
    Request = bytesToHexString(MEPQueue[MEPQueueReplyIndex].Request + 2, MEPQueue[MEPQueueReplyIndex].RequestLength - 8 - 4 - 2, false); // 8=digest, 4=sequence no, 2=length
    SequenceNo = MEPQueue[MEPQueueReplyIndex].Reply[3] * 256 * 256 * 256 +
      ...
      //DGF
      Serial.printf("\033[1;31mCurrent Sequenc no (invalid no. handler): %i\033[0m;\r\n",SequenceNo);
      //DGF
//      SequenceNo++;
      // Repeat last request?
      queueRequest(Request, key, MEPQueue, MEPQueueNextIndex, MEPQueue[MEPQueueReplyIndex].NextAction);
