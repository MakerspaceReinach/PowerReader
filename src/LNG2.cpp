...
LNG2::LNG2(const char* payload, uint8_t useMeterType, MeterConfig* meterConfig, DataParserContext &ctx, RemoteDebug* debugger) {
    ...
    if(h->length == 0x0e) {
        ...
        this->meterCounter_total.activeImportCounter = ntohl(d->acumulatedImport.data) / 1000.0; //DGF changed to 'total'
        this->meterCounter_total.activeExportCounter = ntohl(d->accumulatedExport.data) / 1000.0; //DGF changed to 'total'
