...
HardwareSerial SMSerial(1); //DGF
//HardwareSerial Serial0(0); //DGF
// DGF
...
// DGF 06-DEC-2023 Needed to detect available number of tiers 
extern MeterConfigStruct providerMeterConfig;
// DGF
...
unsigned long mep_config_last_millis = 0;
...
void setup() 
{
	// DGF 11-DEC-2023
	Serial.begin(115200);
	//SMSerial.begin(9600, SERIAL_8N1, 3, 4); // EHorvat NES-MEP
	SMSerial.begin(9600, SERIAL_8N1, D1, D2); // EHorvat NES-MEP, DGF Set to pin, not to GPIO to preserve compatibility of xiao C3 and S3
	...
	ea.setFixedPrice(entsoe.tiers[currentTierId].price / 1000.0, entsoe.currency);
	...
	//DGF 28-AUG-2023
	//Serial.flush();
//	hw.ledBlink(LED_INTERNAL, 5);
	//Serial.end();
	//DGF
	// DGF Removed, serial config is fixed
/* 	if(gpioConfig.hanPin == D2) 
	{
		shared = true;
		#if defined(ESP8266)
		...
		#if defined(ESP32)
		// DGF 22-AUG-2023
			//SMSerial.begin(meterConfig.baud == 0 ? 2400 : meterConfig.baud, serialConfig, 20, 21, meterConfig.invert);
			//SMSerial.begin(9600, SERIAL_8N1, 20, 21);
...
//DGF 28-AUG-2023
 	// if(!shared) {
	// 	Serial.begin(115200);
	// 	Serial.println("Shared = false");
	// }
	// else
	// 	Serial.println("Shared = true");
//DGF
...
//	delay(1000); 			// EHorvat disabled this function
	MEPEnable(true);
    delay(500);
	Serial.println("SerialEvent2 (setup)"); 
	SerialEvent2(); // EHorvat was 1000
 	InputBufferLength = 0;
  // Get started by requesting the UTC Clock
//DGF Removed, because handled inside the loop
...
void SerialEvent2() 
{
	...
	// DGF 22-AUG-2023
	while(SMSerial.available() && InputBufferLength < MaxMEPReplyLength) 
  	//while(Serial2.available() && InputBufferLength < MaxMEPReplyLength) 
	// DGF
	{
	
		// DGF 22-AUG-2023
    	InputBuffer[InputBufferLength] = (byte)SMSerial.read();
...
/***********************************************************************************************************************************************************
 * Purpose: Calculate tier offset of table BT23.
 * History: D.Geisenhoff 06-DEC-2023 Created
 * *********************************************************************************************************************************************************/
int calcBT23TierOffset(byte tierId)
{
	//Serial.println("calcBT23Offset");
	int A = ((providerMeterConfig.BT21_0 & 0b00000100) == 0b00000100) ? 1 : 0; // BT21_0_2
	//Serial.print("A:");
	//Serial.println(A);
	int B = A + 4 * providerMeterConfig.BT21_3;
	//Serial.print("B:");
	//Serial.println(B);
	int SumRcd = providerMeterConfig.BT21_3 * 4;
	//Serial.print("SumRcd:");
	//Serial.println(SumRcd);									  // 4 is size of NI_FMAT2 = 32 bit signed integer
	int DmdRcd = providerMeterConfig.BT21_6 * 5 + 4 + 4 + providerMeterConfig.BT21_6 * 4; // 5 is STIME_DATE = 5 bytes. 4, 4 and 4 are size of NI_FMAT2 = 32 bit signed integer
	//Serial.print("DmdRcd:");
	//Serial.println(DmdRcd);									  // 4 is size of NI_FMAT2 = 32 bit signed integer

	int CoinRcd = providerMeterConfig.BT21_6 * 4;	
	//Serial.print("CoinRcd:");
	//Serial.println(CoinRcd);									  // 4 is size of NI_FMAT2 = 32 bit signed integer
	// DGF 06-DEC-2023 DmdRcd size and CoinRcd size must be multiplied, not added
	int C = B + providerMeterConfig.BT21_4 * DmdRcd;
	//Serial.print("C:");
	//Serial.println(C);
	int D = C + providerMeterConfig.BT21_5 * CoinRcd;
	//Serial.print("D:");
	//Serial.println(D);
	//Serial.println(UnsignedLongAs3ByteHexString(D));
	return D + tierId * (SumRcd + providerMeterConfig.BT21_4 * DmdRcd + providerMeterConfig.BT21_5 * CoinRcd);
}


/***********************************************************************************************************************************************************
 * Purpose: Send config requests to meter once
 * History: D.Geisenhoff 06-DEC-2023 Taken out of loop for better readability
 * *********************************************************************************************************************************************************/
void SendConfigRequestsToMeter()
{
	if(!FirstRequestOk && (millis()-mep_config_last_millis) > 2000 ) 
	//if(!FirstRequestOk ) 
	{// Read UTC clock is first request (queued in Setup)
		mep_config_last_millis = millis();
...
/***********************************************************************************************************************************************************
 * Purpose: Send request to meter or queue requests
 * History: D.Geisenhoff 06-DEC-2023 Taken out of loop for better readability
 * *********************************************************************************************************************************************************/
//byte j;
void SendRequestToMeter()
{
		// Sending packages to meter
  	if(!SentAwaitingReply) 
	{
		// Send request to meter
		//DGF Removed, makes sporadic sequence error in meter (0C)
    	//j = 0;
   		// while((MEPQueue[MEPQueueSendIndex].ReplyLength > 0) && (j <= MaxMEPBuffer)) 
		// {  // (MEPQueue[MEPQueueSendIndex].RequestLength == 0) &&   
      	// 	IncreaseMEPQueueIndex(&MEPQueueSendIndex);
		// 	if(Debug.isActive(RemoteDebug::VERBOSE)) debugD("Increasing index, new index is: %i \r\n", MEPQueueSendIndex);
     	// 	 j++;
    	// }
		//DGF
...
//					if(Debug.isActive(RemoteDebug::VERBOSE)) debugD("This Requests bytes: %i  %X",MEPQueue[MEPQueueSendIndex].Request[i],MEPQueue[MEPQueueSendIndex].Request[i]);    		//EHorvat new
				...
				//DGF
				//Serial.printf("Writing to meter at sendindex: %i,  %s\r\n", MEPQueueNextIndex, RequestsBytesString.c_str());
				//DGF
				for (unsigned long i = 0; i < MEPQueue[MEPQueueSendIndex].RequestLength; i++)
				{
					// DGF 22-AUG-2023
					
					SMSerial.write((byte)MEPQueue[MEPQueueSendIndex].Request[i]);
		...
		{
			// Queue next requests
      		if(millis()-LastSentMillis > 2000 && GetConfigurationRequestsSent) 
			{     //EHorvat Nes-MEP was 5000
			       				 
				// Update consumption data
				// DGF 04-DEC-2023 Get current tariff added
       			queueRequest("300037", mep_key, MEPQueue, &MEPQueueNextIndex, None);
				// DGF 17-NOV-2023: Read only the first 13 bytes of register 0x0017 and remove the MaxMEPReplyLengthAsHex function from both registers.
				//					Read register 0x0017 total values
				MEPQueue[MEPQueueNextIndex].RegisterSpecificParam = 255; // DGF no tier (total)
				queueRequest("3F00170000000011", mep_key, MEPQueue, &MEPQueueNextIndex, None);
				// DGF 06-DEC-2023 Read Register 0x0017 for every tier
 	 			char request[17];
				for (byte tierId = 0; tierId < providerMeterConfig.BT21_7; tierId++)
				{
					MEPQueue[MEPQueueNextIndex].RegisterSpecificParam = tierId; // DGF tier id
					sprintf(request, "3F0017%s0010", UnsignedLongAs3ByteHexString(calcBT23TierOffset(tierId)));
					// Serial.print("Tier request: ");
					// Serial.println(request);
					// // sprintf(offsetStr, "%060d", calcBT23TierOffset(i));
	       			//queueRequest("3F0017000000000D", mep_key, MEPQueue, &MEPQueueNextIndex, None);
					queueRequest(request, mep_key, MEPQueue, &MEPQueueNextIndex, None);
				}  
				queueRequest("30001C", mep_key, MEPQueue, &MEPQueueNextIndex, None);
				// DGF
...
/***********************************************************************************************************************************************************
 * Purpose: Get response from meter.
 * History: D.Geisenhoff 06-DEC-2023 Taken out of loop for better readability
 * *********************************************************************************************************************************************************/
void GetReplyFromMeter()
{
	if (SentAwaitingReply)
	{
...
//      		RS3232Enable(true);		
      		...
      		MEPEnable(true);
      		delay(500);   // EHorvat was 1000
			Serial.println("SerialEvent2 (SentAwaitingDisplay)"); 
      		SerialEvent2();
...
//          			Serial.printf("Error: InputBufferLength: %i, got this reply from the meter, but did not expect one:\r\n",InputBufferLength);	//EHorvat disabled
				...
				{	//DGF more info
					if(Debug.isActive(RemoteDebug::INFO)) debugI("Received: %i, Bytes of Input, index %i, register %i",InputBufferLength,MEPQueueSendIndex,MEPQueue[MEPQueueSendIndex].Request[3] * 256 + MEPQueue[MEPQueueSendIndex].Request[4]); //EHorvat new test
					//DGF
          			...
          			if(!GetConfigurationRequestsSent) 
					{
						...
						{// Read UTC clock is first request (queued in Setup)
							...
							if(Debug.isActive(RemoteDebug::VERBOSE)) 
								debugD("Request response at index %i - Reply#0 to 2= %i %i %i ", MEPQueueSendIndex, MEPQueue[MEPQueueSendIndex].Reply[0], MEPQueue[MEPQueueSendIndex].Reply[1], MEPQueue[MEPQueueSendIndex].Reply[2]); //EHorvat new
            			}         
          			}
          			if(FirstRequestOk) 
					{
...
/***********************************************************************************************************************************************************
 * Purpose: ESP loop (repeatedly called)
...
// EHorvat last_counter_value start
	...
	ImportCounter = meterState.getActiveImportCounterTotal(); //DGF changed to total
	if (ImportCounter < 0) 
	{
		ImportCounter = 1;
	}
	consumption_to_last = int32_t(ImportCounter - ea.getLast_Counter_Value()); // DGF 18-DEC-2023 changed to int32, because of uint, this would never become negative
	...
	ExportCounter = meterState.getActiveExportCounterTotal(); //DGF changed to total
	if (ExportCounter < 0) {
		ExportCounter = 1;
	}
    export_to_last = int32_t(ExportCounter - ea.getLast_expCounter_Value()) ; // DGF 18-DEC-2023 changed to int32, because of uint, this would never become negative
    if (export_to_last < 0) {
        export_to_last = 0;
    }
...
//  ******************   EHorvat NES-MEP added start ******************
	...
	if(millis()- Last_hourplot_millis > 29850) 
	{    // EHorvat set hour plot data update rate of app 30s rate 
...
//
	double activeImportCounter_tiers[8];
	double activeExportCounter_tiers[8];
	double reactiveImportCounter_tiers[8];
	double reactiveExportCounter_tiers[8];
	for(byte tierId = 0; tierId <= 7;tierId++)
	{
		activeImportCounter_tiers[tierId] = ConsumptionData.bt23_tiers[tierId].Fwd_Act_Wh / 1000.0;
		activeExportCounter_tiers[tierId] = ConsumptionData.bt23_tiers[tierId].Rev_Act_Wh / 1000.0;
		reactiveImportCounter_tiers[tierId] = ConsumptionData.bt23_tiers[tierId].Fwd_React_Wh / 1000.0;
		reactiveExportCounter_tiers[tierId] = ConsumptionData.bt23_tiers[tierId].Rev_React_Wh / 1000.0;
	}
	
	ad.setfromNESMEP_1(ConsumptionData.BT28_Fwd_W,
						ConsumptionData.bt23_total.Fwd_Act_Wh / 1000.0,
						ConsumptionData.BT28_Rev_W,
						ConsumptionData.bt23_total.Rev_Act_Wh / 1000.0,
						ConsumptionData.BT28_Fwd_VA,
						ConsumptionData.BT28_Rev_VA,
						ConsumptionData.bt23_total.Fwd_React_Wh / 1000.0,
						ConsumptionData.bt23_total.Rev_React_Wh / 1000.0,
						activeImportCounter_tiers,
						activeExportCounter_tiers,
						reactiveImportCounter_tiers,
						reactiveExportCounter_tiers,
						...
						// DGF 04-DEC-2023 Tariff id added
						ConsumptionData.BT55_Current_Tier_Id
						// DGF
	);
...
{ 
	...
	{ // Must use "isHappy()" in case day state gets reset and lastTimestamp is "now"
...
void configFileParse() 
{
	...
	while((size = file.readBytesUntil('\n', buf, 1024)) > 0) 
	{
		...
		} else if(strncmp_P(buf, PSTR("gpioVccResiGnd "), 19) == 0) {
			if(!lGpio) { config.getGpioConfig(gpio); lGpio = true; };
		...
		// } 
		else if(strncmp_P(buf, PSTR("entsoeTiers "), 12) == 0) 
		{
			if(!lEntsoe) 
			{ 
				config.getEntsoeConfig(entsoe); lEntsoe = true; 
			};
			char *name;
			for (byte tierId = 0; tierId <= 7; tierId++)
			{
				if (tierId == 0)
					...
					name = strtok (NULL, " ");
				strcpy(entsoe.tiers[tierId].name, name);
			}
			char *price;
			for (byte tierId = 0; tierId <= 7; tierId++)
			{
				name = strtok (NULL, " ");
				entsoe.tiers[tierId].price = String(price).toFloat() * 1000;
			}
		} 
		//DGF
			...
			while (pch != NULL) {
				...
				if(day.version < 5) {
					if(i == 0) {
						...
						day.activeImportTotal = val; //DGF changed to total
					} else if(i > 2 && i < 27) {
						day.hImport[i-3] = val / 10;
					} else if(i == 27) {
						day.activeExportTotal = val; //DGF changed to total
					} else if(i > 27 && i < 52) {
		...
		//DGF week plot added
		else if(strncmp_P(buf, PSTR("weekplot "), 9) == 0) 
		{
			int i = 0;
			WeekDataPoints week = { 0 };
			...
			while (pch != NULL) 
			{
				...
				if(week.version < 6) 
				{
					...
					{
						week.lastMeterReadTime = val;
					} 
					else if(i == 2) 
					{
						week.activeImportTotal = val; 
					} 
					else if(i > 2 && i < 10) 
					{
						week.wdImport[i-3] = val / 10;
					} 
					else if(i == 10) 
					{
						week.activeExportTotal = val; 
					} 
					else if(i > 10 && i < 18) 
					{
						week.wdExport[i-11] = val / 10;
					}
				} 
				else 
				{
					...
					{
						week.wdImport[i-4] = val / pow(10, week.accuracy);
					} 
					else if(i == 11) 
					{
						week.activeExportTotal = val; 
					...
					{
						week.wdExport[i-12] = val / pow(10, week.accuracy);
					}
				}
				pch = strtok (NULL, " ");
				i++;
			...
			while (pch != NULL) {
				...
				if(month.version < 6) {
					if(i == 0) {
						...
						month.activeImportTotal = val; //DGF changed to total
					} else if(i > 2 && i < 34) {
						month.dImport[i-3] = val / 10;
					} else if(i == 34) {
						month.activeExportTotal = val; //DGF changed to total
					} else if(i > 34 && i < 66) {
