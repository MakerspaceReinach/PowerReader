...
IEC6205675::IEC6205675(const char* d, uint8_t useMeterType, MeterConfig* meterConfig, DataParserContext &ctx) {
    ...
    if(val == NOVALUE) {
        ...
        if(data->base.type == CosemTypeOctetString) {
            ...
            if(listId.startsWith(F("KFM_001"))) {
                ...
                if(listType == 3) {
                    ...
                    data = getCosemDataAt(idx++, ((char *) (d)));
                    meterCounter_total.activeImportCounter = ntohl(data->dlu.data) / 1000.0; //DGF changed to 'total'
                    data = getCosemDataAt(idx++, ((char *) (d)));
                    meterCounter_total.activeExportCounter = ntohl(data->dlu.data) / 1000.0; //DGF changed to 'total'

                    data = getCosemDataAt(idx++, ((char *) (d)));
                    meterCounter_total.reactiveImportCounter = ntohl(data->dlu.data) / 1000.0; //DGF changed to 'total'
                    data = getCosemDataAt(idx++, ((char *) (d)));
                    meterCounter_total.reactiveExportCounter = ntohl(data->dlu.data) / 1000.0; //DGF changed to 'total'
                }
        ...
        if(val != NOVALUE) {
            listType = 3;
            meterCounter_total.activeImportCounter = val / 1000.0; //DGF changed to 'total'
        }
        val = getNumber(AMS_OBIS_ACTIVE_EXPORT_COUNT, sizeof(AMS_OBIS_ACTIVE_EXPORT_COUNT), ((char *) (d)));
        if(val != NOVALUE) {
            listType = 3;
            meterCounter_total.activeExportCounter = val / 1000.0; //DGF changed to 'total'
        }
        val = getNumber(AMS_OBIS_REACTIVE_IMPORT_COUNT, sizeof(AMS_OBIS_REACTIVE_IMPORT_COUNT), ((char *) (d)));
        if(val != NOVALUE) {
            listType = 3;
            meterCounter_total.reactiveImportCounter = val / 1000.0; //DGF changed to 'total'
        }
        val = getNumber(AMS_OBIS_REACTIVE_EXPORT_COUNT, sizeof(AMS_OBIS_REACTIVE_EXPORT_COUNT), ((char *) (d)));
        if(val != NOVALUE) {
            listType = 3;
            meterCounter_total.reactiveExportCounter = val / 1000.0; //DGF changed to 'total'
        }
        ...
        if(meterType == AmsTypeKamstrup) {
            if(listType >= 3) {
                meterCounter_total.activeImportCounter *= 10; //DGF changed to 'total'
                meterCounter_total.activeExportCounter *= 10; //DGF changed to 'total'
                meterCounter_total.reactiveImportCounter *= 10; //DGF changed to 'total'
                meterCounter_total.reactiveExportCounter *= 10; //DGF changed to 'total'
            }
            if(l1current != 0)
                l1current /= 100;
            if(l2current != 0)
    ...
    {
        meterCounter_total.activeImportCounter = meterCounter_total.activeImportCounter > 0 ? meterCounter_total.activeImportCounter * (meterConfig->accumulatedMultiplier / 1000.0) : 0; //DGF changed to 'total' 
        meterCounter_total.activeExportCounter = meterCounter_total.activeExportCounter > 0 ? meterCounter_total.activeExportCounter * (meterConfig->accumulatedMultiplier / 1000.0) : 0; //DGF changed to 'total'
        meterCounter_total.reactiveImportCounter = meterCounter_total.reactiveImportCounter > 0 ? meterCounter_total.reactiveImportCounter * (meterConfig->accumulatedMultiplier / 1000.0) : 0; //DGF changed to 'total'
        meterCounter_total.reactiveExportCounter = meterCounter_total.reactiveExportCounter > 0 ? meterCounter_total.reactiveExportCounter * (meterConfig->accumulatedMultiplier / 1000.0) : 0; //DGF changed to 'total'
    }
