...
//DGF 14-DEC-2023 Tiers added
MeterCounter NESMEP_MeterCounter_Total;
MeterCounter NESMEP_MeterCounter_Tiers[8];
    ...
    if(other.getListType() < 3) 
    {
        ...
        if(ms > 0) {
            if(other.getActiveImportPower() > 0) 
            {
                ...
                meterCounter_total.activeImportCounter += add / 1000.0;
                meterCounter_tiers[getCurrentTierId()].activeImportCounter += add / 1000.0; //DGF
            ...
            if(other.getListType() > 1) 
            {
                ...
                if(other.getActiveExportPower() > 0) {
                    ...
                    meterCounter_total.activeExportCounter += add / 1000.0;
                    meterCounter_tiers[getCurrentTierId()].activeExportCounter += add / 1000.0; //DGF
                }
                if(other.getReactiveImportPower() > 0) {
                    float add = other.getReactiveImportPower() * (((float) ms) / 3600000.0);
                    meterCounter_total.reactiveImportCounter += add / 1000.0;
                    meterCounter_tiers[getCurrentTierId()].reactiveImportCounter += add / 1000.0; //DGF
                }
                if(other.getReactiveExportPower() > 0) {
                    float add = other.getReactiveExportPower() * (((float) ms) / 3600000.0);
                    meterCounter_total.reactiveExportCounter += add / 1000.0;
                    meterCounter_tiers[getCurrentTierId()].reactiveExportCounter += add / 1000.0; //DGF
                }
            }
...
//        case 3:
            ...
            this->meterCounter_total = NESMEP_MeterCounter_Total;   //DGF
            //DGF
            for (byte tierId; tierId <= 7; tierId++)
            {
                this->meterCounter_tiers[tierId] = NESMEP_MeterCounter_Tiers[tierId]; 
            }
            //DGF
            this->activeImportPower = NESMEP_ActiveImportPower;  //EHorvat NES-MEP
                // this->activeImportCounter = NESMEP_ActiveImportCounter;         //EHorvat NES-MEP, DGF removed
                // this->activeExportCounter = NESMEP_ActiveExportCounter;         //EHorvat NES-MEP, DGF removed
            ...
            //this->reactiveExportCounter = NESMEP_ReactiveExportCounter; //DGF removed
...
uint8_t AmsData::setfromNESMEP_1(long ActiveImportPower,
                                double ActiveImportCounterTotal,
                                long ActiveExportPower, 
                                double ActiveExportCounterTotal, 
                                long ReactiveImportPower, 
                                long ReactiveExportPower, 
                                double ReactiveImportCounterTotal,  
                                double ReactiveExportCounterTotal, 
                                double ActiveImportCounterTier[8],
                                double ActiveExportCounterTier[8], 
                                double ReactiveImportCounterTier[8],  
                                double ReactiveExportCounterTier[8], 
                                float Pwr_Factor_L1,
                                float Pwr_Factor_L2,
                                float Pwr_Factor_L3,
                                long VA_L1L2L3,
...
{
//
    NESMEP_MeterCounter_Total.activeImportCounter = ActiveImportCounterTotal; //DGF 14-DEC-2023 distinct total from tariff
    NESMEP_MeterCounter_Total.activeExportCounter = ActiveExportCounterTotal; //DGF 14-DEC-2023 distinct total from tariff 
    NESMEP_MeterCounter_Total.reactiveImportCounter = ReactiveImportCounterTotal; //DGF 14-DEC-2023 distinct total from tariff
    NESMEP_MeterCounter_Total.reactiveExportCounter = ReactiveExportCounterTotal; //DGF 14-DEC-2023 distinct total from tariff
    //DGF
    for (byte tierId = 0; tierId <= 7; tierId++)
    {
        NESMEP_MeterCounter_Tiers[tierId].activeImportCounter = ActiveImportCounterTier[tierId];
        NESMEP_MeterCounter_Tiers[tierId].activeExportCounter = ActiveExportCounterTier[tierId];
        NESMEP_MeterCounter_Tiers[tierId].reactiveExportCounter = ActiveImportCounterTier[tierId];
        NESMEP_MeterCounter_Tiers[tierId].reactiveExportCounter = ActiveExportCounterTier[tierId];
    }
    // DGF
    ...
    NESMEP_CurrentTierId = CurrentTierId;
    return 1;
}
...
//    return this->listType;
        return 4;  //EHorvat was 3 // DGF was 4
}
...
double AmsData::getActiveImportCounterTotal() //DGF
{
//    return this->activeImportCounter;
    return NESMEP_MeterCounter_Total.activeImportCounter;                  // EHorvat NES-MEP, DGF
}

double AmsData::getReactiveImportCounterTotal() //DGF
{
    return NESMEP_MeterCounter_Total.reactiveImportCounter;  //DGF         
}

double AmsData::getActiveExportCounterTotal() //DGF
{
   return NESMEP_MeterCounter_Total.activeExportCounter;                  // EHorvat NES-MEP
}

double AmsData::getReactiveExportCounterTotal()                         //DGF
{
//    return this->reactiveExportCounter;
    return NESMEP_MeterCounter_Total.reactiveExportCounter;             // Send mep_alivecounter instead of ReactiveExportCounter....
}

//DGF
double AmsData::getActiveImportCounterTier(byte tierId) 
{
//    return this->activeImportCounter;
    return NESMEP_MeterCounter_Tiers[tierId].activeImportCounter;                  
}

double AmsData::getReactiveImportCounterTier(byte tierId) 
{
    return NESMEP_MeterCounter_Tiers[tierId].reactiveImportCounter;      
}

double AmsData::getActiveExportCounterTier(byte tierId) 
{
   return NESMEP_MeterCounter_Tiers[tierId].activeExportCounter;                
}

double AmsData::getReactiveExportCounterTier(byte tierId)                         
{
//    return this->reactiveExportCounter;
    return NESMEP_MeterCounter_Tiers[tierId].reactiveExportCounter;     
}
//DGF

/***********************************************************************************************************************************************************
 * Purpose: Get current tariff id from data structure
 * History: D.Geisenhoff 04-DEC-2023 Created
 * *********************************************************************************************************************************************************/
byte AmsData::getCurrentTierId()
{
    return NESMEP_CurrentTierId;
}
