...
struct EnergyAccountingData 
{
    ...
    uint32_t costThisWeek;      //DGF Week added
    uint32_t costThisMonth;
    uint32_t costLastMonth;
    uint32_t incomeYesterday;
    uint32_t incomeThisWeek;    //DGF Week added
...
public:
    ...
    float getCostThisWeek();  //DGF week added
    ...
    float getIncomeThisWeek();  //DGF week added
...
private:
    ...
    Timezone *tz = NULL;
    uint8_t currentHour = 0, currentDay = 0, currentWeekDay = 0, currentThresholdIdx = 0; //DGF currentWeekDay added
