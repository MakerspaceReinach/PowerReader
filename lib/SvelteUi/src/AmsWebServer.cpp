...
#include "base64.h"
#include "hexutils.h"
#include "AmsData.h"  							//DGF needed for tiers
...
#include "html/weekplot_json.h"					//DGF week plot added
...
extern AmsData ad;									//DGF needed for tiers (current tier)
...
void AmsWebServer::setup(AmsConfiguration* config, GpioConfig* gpioConfig, MeterConfig* meterConfig, AmsData* meterState, AmsDataStorage* ds, EnergyAccounting* ea) {
	...
	server.on(F("/weekplot.json"), HTTP_GET, std::bind(&AmsWebServer::weekplotJson, this));  //DGF week plot added
	...
	//server.on(F("/energyprice.json"), HTTP_GET, std::bind(&AmsWebServer::energyPriceJson, this)); // DGF removed
...
void AmsWebServer::sysinfoJson() {
	...
	int size = snprintf_P(buf, BufferSize, SYSINFO_JSON,
		...
		ui.showDayPlot,
		ui.showWeekPlot,	//DGF week plot added
		ui.showMonthPlot,
...
/***********************************************************************************************************************************************************
 * Purpose: Send data to Web UI
 * History: D.Geisenhoff 19-DEC-2023 This comment added
 * *********************************************************************************************************************************************************/
...
//	} else if(millis - meterState->getLastUpdateMillis() < 15000) {
	...
	else if(millis - mep_Last_ok_millis < 15000) 
	{    			// EHorvat new Timestamp to be used for HAN/MEP port timeout error indication
		hanStatus = 1;  //EHorvat this is to show lable in orange
//	} else if(millis - meterState->getLastUpdateMillis() < 30000) {
	} 
	else if(millis - mep_Last_ok_millis < 30000) 
	{    			// EHorvat new Timestamp to be used for HAN/MEP port timeout error indication
	...
	//DGF tiers added
	byte currentTierId = ad.getCurrentTierId();
	EntsoeConfig entsoeConfig;
	config->getEntsoeConfig(entsoeConfig);
	ea->setFixedPrice(entsoeConfig.tiers[currentTierId].price / 1000.0, entsoeConfig.currency);  // Set import price to price of current tier
	...
	snprintf_P(buf, BufferSize, DATA_JSON,
		...
		//DGF tier data added
		meterState->getCurrentTierId(),
		price == ENTSOE_NO_VALUE ? "null" : String(price, 2).c_str(),
		entsoeConfig.tiers[0].name,
		entsoeConfig.tiers[1].name,
		meterState->getActiveImportCounterTier(0),
		meterState->getActiveImportCounterTier(1),
		//DGF
		...
		meterState->getActiveImportCounterTotal(),  //DGF changed to 'total'
		meterState->getActiveExportCounterTotal(),  //DGF changed to 'total'
		meterState->getReactiveImportCounterTotal(),  //DGF changed to 'total'
		meterState->getReactiveExportCounterTotal(),  //DGF changed to 'total'
...
//		hw->getTemperature(),
		...
		ea->getLast_Counter_Value(),              //EHorvat new last_counter_value ... Link in data.json:"py" : %lu,
		last_cntr_info.c_str(),                   //EHorvat new last_counter_value ... Link in data.json:"pd" : "%s",
		ea->getCostsLastCounterValue(),			  //EHorvat new last_counter_value ... Link in data.json:"pl" : %.2f,
		ea->getLast_Counter_Diff(),               //EHorvat new last_counter_value ... Link in data.json:"pa" : %lu,
		ea->getLast_expCounter_Value(),           //EHorvat new last_counter_value ... Link in data.json:"pz" : %lu,
		last_expcntr_info.c_str(),                //EHorvat new last_counter_value ... Link in data.json:"pg" : "%s",
		ea->getCostsLastexpCounterValue(),		  //EHorvat new last_counter_value ... Link in data.json:"pj" : %.2f,
		ea->getLast_expCounter_Diff(),            //EHorvat new last_counter_value ... Link in data.json:"pk" : %lu,
		meterStateFW.c_str(),
		meterConfig->distributionSystem,
		// Enetrgy accounting start
		ea->getMonthMax(),
		peaks.c_str(),
		...
		ea->getUseThisWeek(),
		ea->getCostThisWeek(),
		ea->getProducedThisWeek(),
		ea->getIncomeThisWeek(),
		//DGF
		ea->getUseThisMonth(),
		ea->getCostThisMonth(),
		ea->getProducedThisMonth(),
		ea->getIncomeThisMonth(),
		//Energy accounting end
...
/***********************************************************************************************************************************************************
 * Purpose: Show week plot
 * History: D.Geisenhoff 21-DEC-2023 Created
 * *********************************************************************************************************************************************************/
void AmsWebServer::weekplotJson() 
{
	if(debugger->isActive(RemoteDebug::INFO)) debugger->printf_P(PSTR("Serving /weekplot.json over http...\n")); //EHorvat changed to Info level
	if(!checkSecurity(2))
		return;
	//if(debugger->isActive(RemoteDebug::INFO)) debugger->printf_P(PSTR("Buffer length actual of week.json...is: %d, \n"), strlen(buf));//EHorvat, DGF removed
	if(ds == NULL) 
	{
		notFound();
	} 
	...
	{
		snprintf_P(buf, BufferSize, WEEKPLOT_JSON,
			ds->getWeekDayImport(1) / 1000.0,
			ds->getWeekDayImport(2) / 1000.0,
			ds->getWeekDayImport(3) / 1000.0,
			ds->getWeekDayImport(4) / 1000.0,
			ds->getWeekDayImport(5) / 1000.0,
			ds->getWeekDayImport(6) / 1000.0,
			ds->getWeekDayImport(7) / 1000.0,
			ds->getWeekDayExport(1) / 1000.0,
			ds->getWeekDayExport(2) / 1000.0,
			ds->getWeekDayExport(3) / 1000.0,
			ds->getWeekDayExport(4) / 1000.0,
			ds->getWeekDayExport(5) / 1000.0,
			ds->getWeekDayExport(6) / 1000.0,
			ds->getWeekDayExport(7) / 1000.0
		);
		server.sendHeader(HEADER_CACHE_CONTROL, CACHE_CONTROL_NO_CACHE);
		server.sendHeader(HEADER_PRAGMA, PRAGMA_NO_CACHE);
		server.sendHeader(HEADER_EXPIRES, EXPIRES_OFF);
...
/***********************************************************************************************************************************************************
 * Purpose: Show configuration data on Web UI
 * History: D.Geisenhoff 19-DEC-2023 This comment added
 * *********************************************************************************************************************************************************/
void AmsWebServer::configurationJson() 
{
	...
	server.sendContent(buf);
	//Serial.printf("Tier0 name:%s, length: %i\n", entsoe.tiers[0].name, strlen(entsoe.tiers[0].name));
	snprintf_P(buf, BufferSize, CONF_PRICE_JSON,
			...
			//entsoe.fixedPrice == 0 ? "null" : String(entsoe.fixedPrice / 1000.0, 10).c_str(), //DGF replaced by tiers
			   // DGF tiers added
			   strncpy(entsoe.tiers[0].name, entsoe.tiers[0].name, 10),      //DGF limit to 10 chars
			   strncpy(entsoe.tiers[1].name, entsoe.tiers[1].name, 10),
			   strncpy(entsoe.tiers[2].name, entsoe.tiers[2].name, 10),
			   strncpy(entsoe.tiers[3].name, entsoe.tiers[3].name, 10),
		       strncpy(entsoe.tiers[4].name, entsoe.tiers[4].name, 10),
			   strncpy(entsoe.tiers[5].name, entsoe.tiers[5].name, 10),
			   strncpy(entsoe.tiers[6].name, entsoe.tiers[6].name, 10),
			   strncpy(entsoe.tiers[7].name, entsoe.tiers[7].name, 10),
			   entsoe.tiers[0].price == 0 ? "null" : String(entsoe.tiers[0].price / 1000.0, 10).c_str(),
			   entsoe.tiers[1].price == 0 ? "null" : String(entsoe.tiers[1].price / 1000.0, 10).c_str(),
			   entsoe.tiers[2].price == 0 ? "null" : String(entsoe.tiers[2].price / 1000.0, 10).c_str(),
			   entsoe.tiers[3].price == 0 ? "null" : String(entsoe.tiers[3].price / 1000.0, 10).c_str(),
			   entsoe.tiers[4].price == 0 ? "null" : String(entsoe.tiers[4].price / 1000.0, 10).c_str(),
			   entsoe.tiers[5].price == 0 ? "null" : String(entsoe.tiers[5].price / 1000.0, 10).c_str(),
			   entsoe.tiers[6].price == 0 ? "null" : String(entsoe.tiers[6].price / 1000.0, 10).c_str(),
			   entsoe.tiers[7].price == 0 ? "null" : String(entsoe.tiers[7].price / 1000.0, 10).c_str(),
			   // DGF
...
/***********************************************************************************************************************************************************
 * Purpose: Get configuration data from web UI and save.
...
void AmsWebServer::handleSave() {
	...
	bool success = true;
	//DGF removed, board is fixed (xiao)
/* 	if(server.hasArg(F("v")) && server.arg(F("v")) == F("true")) {
	...
	//DGF removed, gpio config is fixed by hardware
	...
	if(server.hasArg(F("u")) && server.arg(F("u")) == F("true")) {
		...
		//ui.showPricePlot = server.arg(F("up")).toInt();  //DGF removed
		...
		ui.showWeekPlot = server.arg(F("uw")).toInt();  // DGF Week plot added
...
//		ui.showTemperaturePlot = server.arg(F("us")).toInt();
	...
	if(server.hasArg(F("p")) && server.arg(F("p")) == F("true")) {
		...
		entsoe.multiplier = server.arg(F("pm")).toFloat() * 1000;
		 // DGF tiers added
		entsoe.tiers[0].price = server.arg(F("pf0")).toFloat() * 1000;
		entsoe.tiers[1].price = server.arg(F("pf1")).toFloat() * 1000;
		entsoe.tiers[2].price = server.arg(F("pf2")).toFloat() * 1000;
		entsoe.tiers[3].price = server.arg(F("pf3")).toFloat() * 1000;
		entsoe.tiers[4].price = server.arg(F("pf4")).toFloat() * 1000;
		entsoe.tiers[5].price = server.arg(F("pf5")).toFloat() * 1000;
		entsoe.tiers[6].price = server.arg(F("pf6")).toFloat() * 1000;
		entsoe.tiers[7].price = server.arg(F("pf7")).toFloat() * 1000;
		strncpy(entsoe.tiers[0].name, server.arg(F("pn0")).c_str(), 10);
		strncpy(entsoe.tiers[1].name, server.arg(F("pn1")).c_str(), 10);
		strncpy(entsoe.tiers[2].name, server.arg(F("pn2")).c_str(), 10);
		strncpy(entsoe.tiers[3].name, server.arg(F("pn3")).c_str(), 10);
		strncpy(entsoe.tiers[4].name, server.arg(F("pn4")).c_str(), 10);
		strncpy(entsoe.tiers[5].name, server.arg(F("pn5")).c_str(), 10);
		strncpy(entsoe.tiers[6].name, server.arg(F("pn6")).c_str(), 10);
		strncpy(entsoe.tiers[7].name, server.arg(F("pn7")).c_str(), 10);
		//DGF	
		...
		strncpy(entsoe.last_counter_info, server.arg(F("pd")).c_str(), 15);      //EHorvat new last_counter_value limited to 16 char, DGF limited to 15 char
		entsoe.last_expcounter_value = server.arg(F("pz")).toInt();		         //EHorvat new last_counter_value
		strncpy(entsoe.last_expcounter_info, server.arg(F("pg")).c_str(), 15);   //EHorvat new last_counter_value limited to 16 char, DGF limited to 15 char
...
void AmsWebServer::configFileDownload() {
	...
	//bool includeWeb = server.hasArg(F("ie")) && server.arg(F("ie")) == F("true"); // DGF removed
	bool includeUI = server.hasArg(F("iu")) && server.arg(F("iu")) == F("true");  //DGF added UI
	bool includeMeter = server.hasArg(F("it")) && server.arg(F("it")) == F("true");
	//bool includeGpio = server.hasArg(F("ig")) && server.arg(F("ig")) == F("true"); // DGF removed
	...
	//DGF removed (always save security settings)
	...
	// {
		...
		if(web.security > 0 && includeSecrets) //DGF includeSecrets added
		{
	...
	if(includeEntsoe) 
	{
		...
		server.sendContent(buf, snprintf_P(buf, BufferSize, PSTR("entsoeTiers %s %s %s %s %s %s %s %s %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f\n"),
										   entsoe.tiers[0].name,
										   entsoe.tiers[1].name,
										   entsoe.tiers[2].name,
										   entsoe.tiers[3].name,
										   ...
										   entsoe.tiers[6].name,
										   entsoe.tiers[7].name,
										   entsoe.tiers[0].price / 1000.0,
										   entsoe.tiers[1].price / 1000.0,
										   entsoe.tiers[2].price / 1000.0,
										   entsoe.tiers[3].price / 1000.0,
										   entsoe.tiers[4].price / 1000.0,
										   entsoe.tiers[5].price / 1000.0,
										   entsoe.tiers[6].price / 1000.0,
										   entsoe.tiers[7].price / 1000.0));
		// DGF
	...
	if(ds != NULL) 
	{
		...
		server.sendContent(buf, snprintf_P(buf, BufferSize, PSTR("dayplot %d %lu %lu %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d"), 
			...
			day.activeImportTotal,  //DGF changed to 'total'
			day.accuracy,
			ds->getHourImport(0),
			ds->getHourImport(1),
			ds->getHourImport(2),
		...
		//DGF week plot added
		WeekDataPoints week = ds->getWeekData();
		server.sendContent(buf, snprintf_P(buf, BufferSize, PSTR("weekplot %d %lu %lu %d %d %d %d %d %d %d"), 
			week.version,
			(int32_t) week.lastMeterReadTime,
			week.activeImportTotal,  //DGF changed to 'total'
			week.accuracy,
			ds->getDayImport(1),
			ds->getDayImport(2),
			ds->getDayImport(3),
		...
		if(week.activeExportTotal > 0)  //DGF changed to 'total'
		{
			server.sendContent(buf, snprintf_P(buf, BufferSize, PSTR(" %u %d %d %d %d %d %d %d\n"), 
				week.activeExportTotal,  //DGF changed to 'total'
				ds->getDayExport(1),
				ds->getDayExport(2),
				ds->getDayExport(3),
				ds->getDayExport(4),
		...
		server.sendContent(buf, snprintf_P(buf, BufferSize, PSTR("monthplot %d %lu %lu %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d"), 
			...
			month.activeImportTotal,  //DGF changed to 'total'
			month.accuracy,
			ds->getDayImport(1),
			ds->getDayImport(2),
			ds->getDayImport(3),
	...
	if(ea != NULL) 
	{
		...
		server.sendContent(buf, snprintf_P(buf, BufferSize, PSTR("energyaccounting %d %d %.2f %.2f %.2f %.2f %.2f %.2f %.2f %d %.2f %d %.2f %d %.2f %d %.2f %d %.2f %.2f %.2f"), 
			ead.version,
			ead.month,
			ea->getCostYesterday(),
			ea->getCostThisWeek(), // DGF Week added
			ea->getCostThisMonth(),
			ea->getCostLastMonth(),
