...
<div class="grid xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2">
    <div class="cnt">
        <form on:submit|preventDefault={handleSubmit}>
            ...
            <strong class="text-sm">Einrichten</strong>
            <div class="my-3">
                SSID<br/>
                <input name="ss" type="text" class="in-s" required maxlength="32"/> <!--DGF maxlength added-->
            </div>
            <div class="my-3">
                Passwort<br/>
            ...
            <div>
                Hostname
                <input name="sh" bind:value={sysinfo.hostname} type="text" class="in-s" maxlength="32" pattern="[A-Za-z0-9_-]+" placeholder="Optional, bsp.: stromzaehler" autocomplete="off"/>
            </div>
            <div class="my-3">
                <label><input type="checkbox" name="sm" value="static" class="rounded mb-1" bind:checked={staticIp} /> Statische IP</label>
                {#if staticIp}
                <br/>
                <div class="flex">
...
<Mask active={loadingOrSaving} message="Deine Konfiguration wird auf das Gerät gespeichert"/>
