...
export const dataStore = readable(data, (set) => { 
 ...
 //                   setTimeout(getTemperatures, 2000);
                ...
                else if (!sysinfo || !sysinfo.chip || sysinfo.booting || (tries > 1 && !isBusPowered(sysinfo.board)))
                {
                    ...
                    //DGF weekplot added
                    if(weekPlotTimeout) clearTimeout(weekPlotTimeout);
                    weekPlotTimeout = setTimeout(getWeekPlot, 3000);     
                    //DGF
...
let weekPlot = {};
let weekPlotTimeout;
export async function getWeekPlot()
{
    if (weekPlotTimeout)
    {
        clearTimeout(weekPlotTimeout);
        weekPlotTimeout = 0;
    }
    const response = await fetchWithTimeout("/weekplot.json");
    weekPlot = (await response.json())
    weekPlotStore.set(weekPlot);

    let date = new Date();
    weekPlotTimeout = setTimeout(getWeekPlot, ((24 - date.getHours()) * 3600000) + 40);
    //weekPlotTimeout = setTimeout(getWeekPlot, 5000);
}
export const weekPlotStore = writable(weekPlot, (set) =>
{ 
    getWeekPlot();
    return function stop() {}
});
