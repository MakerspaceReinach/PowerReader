...
bool AmsConfiguration::setWiFiConfig(WiFiConfig& config) {
	...
	stripNonAscii((uint8_t*) config.ssid, 33, true);
	stripNonAscii((uint8_t*) config.psk, 64,false);
...
bool AmsConfiguration::setMqttConfig(MqttConfig& config) {
	...
	stripNonAscii((uint8_t*) config.host, 128,false);
...
/***********************************************************************************************************************************************************
 * Purpose: Read GPIO configuration from storage
...
bool AmsConfiguration::getGpioConfig(GpioConfig& config) {
	...
	if(configVersion == EEPROM_CHECK_SUM || configVersion == EEPROM_CLEARED_INDICATOR) 
	{
		...
		//DGF fixed for xiao c3 and s3
		config.apPin = D10;	
		config.ledPin = D0;
		config.ledInverted = false;
		//DGF
		return true;
	} 
	else 
...
/***********************************************************************************************************************************************************
 * Purpose: Save GPIO configuration to storage
 * History: D.Geisenhoff 18-DEC-2023 This comment added
 * *********************************************************************************************************************************************************/
...
{
	config.hanPin = D1;  //EHorvat set 16 for UART2 on ESP32 (was 3), DGF was 16
	config.hanPinPullup = true;
	config.apPin = D10; //EHorvat set AP GPIO to 26 (was 0xFF), DGF set to D10, was 26
	config.ledPin = 0xFF;
...
bool AmsConfiguration::setNtpConfig(NtpConfig& config) {
	...
	stripNonAscii((uint8_t*) config.server, 64,false);
	stripNonAscii((uint8_t*) config.timezone, 32,false);
...
bool AmsConfiguration::setEntsoeConfig(EntsoeConfig& config) 
{
	...
	if(getEntsoeConfig(existing)) 
	{
		...
		//entsoeChanged |= config.fixedPrice != existing.fixedPrice; //DGF replaced by tiers
		...
		{
			entsoeChanged |= strcmp(config.tiers[tierId].name, existing.tiers[tierId].name) != 0;  
			entsoeChanged |= config.tiers[tierId].price != existing.tiers[tierId].price;   
		}
		//DGF
	} 
	else 
	...
	for (byte tierId = 0; tierId <= 7; tierId++)
	{
		stripNonAscii((uint8_t*) config.tiers[tierId].name,11);
	}
	//DGF
...
void AmsConfiguration::clearEntsoe(EntsoeConfig& config) 
{
	...
	//config.fixedPrice = 0;  //DGF replaced by tiers
	...
	for (byte tierId = 0; tierId <= 7; tierId++)
	{
		strcpy(config.tiers[tierId].name, "");  
		config.tiers[tierId].price = 0;   
	}	
	//DGF
}
...
void AmsConfiguration::clearUiConfig(UiConfig& config) {
	...
	config.showWeekPlot = 1;  //DGF week plot added
...
bool AmsConfiguration::relocateConfig102() {
	...
	EntsoeConfig entsoe;
	EEPROM.get(CONFIG_ENTSOE_START, entsoe);
	entsoe.tiers[0].price = 0;  // DGF tiers added
	EEPROM.put(CONFIG_ENTSOE_START, entsoe);
...
/***********************************************************************************************************************************************************
 * Purpose: Relocate old config on EEPROM (prices by tariff added)
 * History: D.Geisenhoff 18-DEC-2023 This comment added
 * *********************************************************************************************************************************************************/
...
{		
	...
	MqttConfig mqtt;
	EEPROM.get(1046, mqtt);

	HomeAssistantConfig haconf;
	EEPROM.get(1722, haconf);
