...
/***********************************************************************************************************************************************************
 * Purpose: Handler for sending raw MQTT data to the broker
...
bool RawMqttHandler::publish(AmsData* data, AmsData* meterState, EnergyAccounting* ea, EntsoeApi* eapi) {
    ...
    {
        //Serial.println("publish raw");
        mqtt->publish(topic + "/meter/dlms/timestamp", String(data->getPackageTimestamp()));
    }
    // Send data of all 4 lists to mqtt, if getListType = 4
...
// full = 2 second rate
// 
    // DGF 02-DEC-2023 Added system to uptime topic
    if(full || mqtt10sec) 
    {
        mqtt->publish(topic + "/system/uptime", String((unsigned long) millis64()/1000));  //EHorvat 
        // DGF
...
//        mqtt->publish(topic + "/meter/type", data->getMeterModel(), true, 0);
        ...
        mqtt->publish(topic + "/meter/import/active/accumulated/total", String(data->getActiveImportCounterTotal(), 3), true, 0); //DGF changed to total
        mqtt->publish(topic + "/meter/import/reactive/accumulated/total", String(data->getReactiveImportCounterTotal(), 3), true, 0); //DGF changed to total
        mqtt->publish(topic + "/meter/export/active/accumulated/total", String(data->getActiveExportCounterTotal(), 3), true, 0); //DGF changed to total
        mqtt->publish(topic + "/meter/export/reactive/accumulated/total", String(data->getReactiveExportCounterTotal(), 3), true, 0); //DGF changed to total
        //DGF
        for (byte tierId = 0; tierId <= 7; tierId++)
        {
            double activeImportCounter = data->getActiveImportCounterTier(tierId);
            double activeExportCounter = data->getActiveExportCounterTier(tierId);
            double reactiveImportCounter = data->getReactiveImportCounterTier(tierId);
            double reactiveExportCounter = data->getReactiveExportCounterTier(tierId);
          
            if (activeImportCounter + activeExportCounter + reactiveImportCounter + reactiveExportCounter > 0.0)
            {

                char tierIdStr[1];
				sprintf(tierIdStr, "%i", tierId);
                mqtt->publish(topic + "/meter/import/active/accumulated/tariffs/"+tierIdStr, String(activeImportCounter, 3), true, 0); 
                mqtt->publish(topic + "/meter/export/active/accumulated/tariffs/"+tierIdStr, String(activeExportCounter, 3), true, 0); 
                mqtt->publish(topic + "/meter/import/reactive/accumulated/tariffs/"+tierIdStr, String(reactiveImportCounter, 3), true, 0); 
                mqtt->publish(topic + "/meter/export/reactive/accumulated/tariffs/"+tierIdStr, String(reactiveExportCounter, 3), true, 0); 
            }
        }
        //DGF
    }
...
bool RawMqttHandler::publishList4(AmsData* data, AmsData* meterState) {
    if(full || mqtt10sec) {
            ...
            mqtt->publish(topic + "/meter/currenttariff", String(data->getCurrentTierId()));
            // DGF
        }
        return true;
}
...
bool RawMqttHandler::publishSystem(HwTools* hw, EntsoeApi* eapi, EnergyAccounting* ea) {
	if(topic.isEmpty() || !mqtt->connected())
		return false;
    // DGF 02-DEC-2022 system aded to topic
	mqtt->publish(topic + "/system/mac", WiFi.macAddress(), true, 0);
//	mqtt->publish(topic + "/uptime", String((unsigned long) millis64()/1000)); //EHorvat moved this line to publishList1
	float vcc = hw->getVcc();
	if(vcc > 0) {
		mqtt->publish(topic + "/system/vcc", String(vcc, 2));
	}
	mqtt->publish(topic + "/system/mem", String(ESP.getFreeHeap()));
	mqtt->publish(topic + "/system/rssi", String(hw->getWifiRssi()));
    // DGF
